...
-- Clean up and recreate the handle_new_user function
...
BEGIN
  ...
  v_client_status := COALESCE(NEW.raw_user_meta_data->>'client_status', 'New');
...
-- Create the trigger to fire after INSERT on auth.users
...
VALUES (
  '20250610_fix_handle_new_user',
  'Fixed handle_new_user function and recreated trigger',
  jsonb_build_object('action', 'fix_trigger')
);
