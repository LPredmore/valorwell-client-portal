...
const Login = () => {
  ...
  const handleSuccessfulLogin = () => {
    DebugUtils.log(sessionId, "Handling successful login", {
      clientStatus
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      DebugUtils.log(sessionId, "Attempting login with", {
        email
      });
      const {
        success,
      ...
      DebugUtils.log(sessionId, "Login result", {
        success,
        error
      });
      if (success) {
  ...
  return <div className="min-h-screen flex items-center justify-center bg-gray-100">
        ...
        <CardContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-between items-center">
              ...
              <button type="button" onClick={() => navigate("/reset-password")} className="text-sm text-green-600 hover:text-green-800 font-medium">Password Reset</button>
            </div>
          ...
          {showDebug && <div className="mt-2 p-3 bg-gray-50 border border-gray-200 rounded-md">
              ...
              <Button type="button" size="sm" variant="outline" className="mt-2 text-xs w-full" onClick={handleDiagnosticsPageOpen}>
      ...
      <ForgotPasswordDialogEnhanced isOpen={showForgotPassword} onOpenChange={setShowForgotPassword} />
    </div>;
};
